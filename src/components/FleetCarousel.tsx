    ...
    <div className="w-full">
      ...
      <div key={aircraft.code} className="mt-3 text-center animate-fade-in">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
          <StatTile delay={0} icon={Users} label={t('fleet.seats')} value={String(totalSeats(aircraft.seats))} />
          <StatTile delay={60} icon={Ruler} label={t('fleet.range')} value={`${aircraft.rangeKm.toLocaleString()} km`} />
          <StatTile delay={120} icon={Gauge} label={t('fleet.speed')} value={`${aircraft.cruiseSpeedKmh} km/h`} />
          <StatTile delay={180} icon={MoveHorizontal} label={t('fleet.wingspan')} value={`${aircraft.wingspanM} m`} />
        </div>
      </div>
    </div>
  );
