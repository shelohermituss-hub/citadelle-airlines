    ...
    <div className="w-full">
      ...
      <div className="mt-3 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
          <Spec icon={Users} label={t('fleet.seats')} value={String(totalSeats(aircraft.seats))} />
          <Spec icon={Ruler} label={t('fleet.range')} value={`${aircraft.rangeKm.toLocaleString()} km`} />
          <Spec icon={Gauge} label={t('fleet.speed')} value={`${aircraft.cruiseSpeedKmh} km/h`} />
          <Spec icon={MoveHorizontal} label={t('fleet.wingspan')} value={`${aircraft.wingspanM} m`} />
        </div>
      </div>
    </div>
  );
...
function Spec({
  ...
  return (
    <div className="flex flex-col items-center gap-1 rounded-xl bg-citadelle-cream py-2.5 px-2">
      <Icon className="h-4 w-4 text-citadelle-gold-dark" />
